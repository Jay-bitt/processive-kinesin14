function [M1,m1,h1,state,lengthx,x,x1,x2,x3,x4,x5,x21,x22,x31,x32] = testMp(M1,m1,h1,state,N,N1,dn0,lengthx,x,x1,x2,x3,x4,x5,x21,x22,x31,x32)
dt = 1e-6;
kcat = 9*dt; kon = 10*dt; kd = 300*dt; kdiff = 6000*dt; k0 = 3700*dt; E = 5; K = 0.2; Fx = 0; Np = 4;  r0 = 62.2;
K = K/4.1; 
    ...
    elseif state(i+dn) == 22
        ...
        elseif m1(i)-1 > 0 && rand() < kdiff*exp(-0.5*E)% && M(m1(i)+1) == 0 
            ...
            if rand() < k0*exp(-0.5*(E2-E1))
                x = x + position_right; x2 = x2 + position_right; x22 = x22 + position_right;
                state(i+dn) = 22;
    ...
    elseif state(i+dn) == 21
        ...
        elseif m1(i)+1 < N && rand() < kdiff*exp(0.5*E)% && M(m1(i)-1) == 0
            ...
            if rand() < k0*exp(-0.5*(E0-E1))
                x = x + position_left; x2 = x2 + position_left; x21 = x21 + position_left;
                state(i+dn) = 22;
                lengthx = length_left;
            end    
        end
